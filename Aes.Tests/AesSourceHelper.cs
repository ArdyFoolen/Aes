...
namespace Aes.Tests
{
    public static class AesSourceHelper
    {
        ...
        private static byte[] IVCfb = new byte[]
        {
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F
        };
        public static byte[][] KeysCfb = new byte[3][]
        {
            new byte[] { 0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6, 0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C },
            new byte[] { 0x8E, 0x73, 0xB0, 0xF7, 0xDA, 0x0E, 0x64, 0x52, 0xC8, 0x10, 0xF3, 0x2B, 0x80, 0x90, 0x79, 0xE5, 0x62, 0xF8, 0xEA, 0xD2, 0x52, 0x2C, 0x6B, 0x7B },
            new byte[] { 0x60, 0x3D, 0xEB, 0x10, 0x15, 0xCA, 0x71, 0xBE, 0x2B, 0x73, 0xAE, 0xF0, 0x85, 0x7D, 0x77, 0x81, 0x1F, 0x35, 0x2C, 0x07, 0x3B, 0x61, 0x08, 0xD7, 0x2D, 0x98, 0x10, 0xA3, 0x09, 0x14, 0xDF, 0xF4 }
        };
        public static byte[][] KeysOfb = new byte[3][]
        {
            new byte[] { 0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6, 0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C },
            new byte[] { 0x8E, 0x73, 0xB0, 0xF7, 0xDA, 0x0E, 0x64, 0x52, 0xC8, 0x10, 0xF3, 0x2B, 0x80, 0x90, 0x79, 0xE5, 0x62, 0xF8, 0xEA, 0xD2, 0x52, 0x2C, 0x6B, 0x7B },
            new byte[] { 0x60, 0x3D, 0xEB, 0x10, 0x15, 0xCA, 0x71, 0xBE, 0x2B, 0x73, 0xAE, 0xF0, 0x85, 0x7D, 0x77, 0x81, 0x1F, 0x35, 0x2C, 0x07, 0x3B, 0x61, 0x08, 0xD7, 0x2D, 0x98, 0x10, 0xA3, 0x09, 0x14, 0xDF, 0xF4 }
        ...
        public static IEnumerable<(byte[] In, byte[] Out, Action<Aes.AF.AesManager, Stream, Stream> Crypt)> EncryptDecryptCFB
        {
            get
            {
                ...
                cryptBytes = new byte[] { 0xCD, 0xC8, 0x0D, 0x6F, 0xDD, 0xF1, 0x8C, 0xAB, 0x34, 0xC2, 0x59, 0x09, 0xC9, 0x9A, 0x41, 0x74,
                ...
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.OneHundredTwentyEight, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.OneHundredTwentyEight, outStream, inStream));

                cryptBytes = new byte[] { 0xDC, 0x7E, 0x84, 0xBF, 0xDA, 0x79, 0x16, 0x4B, 0x7E, 0xCD, 0x84, 0x86, 0x98, 0x5D, 0x38, 0x60,
                ...
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.OneHundredTwentyEight, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.OneHundredTwentyEight, outStream, inStream));

                //------------------------------------------------------------------------------------------------------------------------

                plainBytes = new byte[] { 0x6B, 0xC1, 0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D, 0x7E, 0x11, 0x73, 0x93, 0x17, 0x2A };
                cryptBytes = new byte[] { 0x3B, 0x79, 0x42, 0x4C, 0x9C, 0x0D, 0xD4, 0x36, 0xBA, 0xCE, 0x9E, 0x0E, 0xD4, 0x58, 0x6A, 0x4F };
                ...
                cryptBytes = new byte[] { 0xCD, 0xA2, 0x52, 0x1E, 0xF0, 0xA9, 0x05, 0xCA, 0x44, 0xCD, 0x05, 0x7C, 0xBF, 0x0D, 0x47, 0xA0 };
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.Eight, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.Eight, outStream, inStream));

                cryptBytes = new byte[] { 0xDC, 0x1F, 0x1A, 0x85, 0x20, 0xA6, 0x4D, 0xB5, 0x5F, 0xCC, 0x8A, 0xC5, 0x54, 0x84, 0x4E, 0x88 };
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.Eight, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.Eight, outStream, inStream));
                ...
                plainBytes = new byte[] { 0x6A, 0x59 };
                cryptBytes = new byte[] { 0x69, 0xC8 };
                ...
                plainBytes = new byte[] { 0x6E, 0xE0 };
                cryptBytes = new byte[] { 0x97, 0x76 };
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.One, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[1], IVCfb, AesKeySize.Aes192, FeedbackSizeEnum.One, outStream, inStream));

                plainBytes = new byte[] { 0x69, 0xF4 };
                cryptBytes = new byte[] { 0x93, 0xD0 };
                yield return (plainBytes, cryptBytes, (aes, outStream, inStream) => EncryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.One, outStream, inStream));
                yield return (cryptBytes, plainBytes, (aes, outStream, inStream) => DecryptCFB(aes, KeysCfb[2], IVCfb, AesKeySize.Aes256, FeedbackSizeEnum.One, outStream, inStream));
            }
        }

        public static IEnumerable<(byte[] In, byte[] Out, Action<Aes.AF.AesManager, Stream, Stream> Crypt)> EncryptDecryptOFB
        {
            get
            {
                byte[] plainBytes = new byte[] { 0x6B, 0xC1, 0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D, 0x7E, 0x11, 0x73, 0x93, 0x17, 0x2A,
        ...
        private static void EncryptCFB(AF.AesManager aesManager, byte[] key, byte[] IV, AesKeySize keySize, FeedbackSizeEnum feedbackSize, Stream outStream, Stream inStream)
        {
            using (var encryptStream = new CryptoStream(outStream, aesManager.CreateCfbEncryptor(key, IV, keySize, feedbackSize), CryptoStreamMode.Write, true))
            {
                encryptStream.WriteFrom(inStream);
            }
        }

        private static void DecryptCFB(AF.AesManager aesManager, byte[] key, byte[] IV, AesKeySize keySize, FeedbackSizeEnum feedbackSize, Stream outStream, Stream inStream)
        {
            using (var encryptStream = new CryptoStream(inStream, aesManager.CreateCfbDecryptor(key, IV, keySize, feedbackSize), CryptoStreamMode.Read, true))
            {
                encryptStream.ReadInto(outStream);
            }
        }

        private static void EncryptOFB(AF.AesManager aesManager, byte[] key, byte[] IV, AesKeySize keySize, Stream outStream, Stream inStream)
        {
            using (var encryptStream = new CryptoStream(outStream, aesManager.CreateOfbEncryptor(key, IV, keySize), CryptoStreamMode.Write, true))
            {
                encryptStream.WriteFrom(inStream);
            }
        }

        private static void DecryptOFB(AF.AesManager aesManager, byte[] key, byte[] IV, AesKeySize keySize, Stream outStream, Stream inStream)
        {
            using (var encryptStream = new CryptoStream(inStream, aesManager.CreateOfbEncryptor(key, IV, keySize), CryptoStreamMode.Read, true))
            {
                encryptStream.ReadInto(outStream);
            }
        }
