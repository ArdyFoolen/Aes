...
namespace Aes.AF
{
    ...
    public partial class AesManager
    {
        protected class Aes
        {
            ...
            public Aes(byte[] byteKey, AesKeySize keySize = AesKeySize.Aes128)
            {
            ...
            public Aes(byte[] byteKey, byte[] IV, AesKeySize keySize = AesKeySize.Aes128)
            {
                this.KeySize = keySize;
                int kSz = GetKeySize();
                if (kSz != byteKey.Length)
            ...
            public EncryptModeEnum EncryptMode { get; set; } = EncryptModeEnum.ECB;
